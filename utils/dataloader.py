...
def get_im_gt_name_dict(datasets, flag='valid', dataset_size=None):
    """dataset_size: number of images to be used in each dataset, if None, use all images in each dataset
    ...
    for i in range(len(datasets)):
        ...
        name_im_gt_list.append({"dataset_name":datasets[i]["name"],
                                "im_path":tmp_im_list[:dataset_size],
                                "gt_path":tmp_gt_list[:dataset_size],
                                "im_ext":datasets[i]["im_ext"],
                                "gt_ext":datasets[i]["gt_ext"]})

    return name_im_gt_list

def create_dataloaders(name_im_gt_list, my_transforms=[], batch_size=1, training=False, eval_ori_resolution=True):
    gos_dataloaders = []
    gos_datasets = []
    ...
    if training:
        ...
        for i in range(len(name_im_gt_list)):   
            # 在需要evaluate时batchsize大于1时，这时就是为了用这个batch数据，而不是真的evaluate性能(用在show img时)，就设置eval_ori_resolution = False。其他时候记得调回来
            gos_dataset = OnlineDataset([name_im_gt_list[i]], transform = transforms.Compose(my_transforms), eval_ori_resolution=eval_ori_resolution)
            sampler = DistributedSampler(gos_dataset, shuffle=True)
...
class LargeScaleJitter(object):
    ...
    def __call__(self, sample):
        ...
        #resize keep ratio, 把长边resize到1024
...
class OnlineDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        "imidx": torch.tensor(self.dataset_name_map[self.dataset["data_name"][idx]]), # 换成dataset name，本来想添加一个key，但是transform中已经写死了这4个，我就直接替换懒得改了
        "image": im,
        "label": gt,
